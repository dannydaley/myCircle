...
const GET_MESSAGES_BY_CHAT_ID = "SELECT * FROM `messages` WHERE chatId = ? ORDER BY date";
...
app.post('/getChat', (req, res) => {
  ...
  SQLdatabase.all(GET_CHAT_DATA_BY_CHAT_ID, [user, user, req.body.chatId], (err, chatData) => {
    // if error
    if (err) {
      // respond with error status and error message
      res.status(500).send(err.message);
